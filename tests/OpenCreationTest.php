...
class OpenCreationTest extends \PHPUnit_Framework_TestCase
{
    ...
    */
    ...
    /** @test */
    public function it_uses_open_creation_with_a_raw_dictionary_of_closures_within_the_main_constructor()
    {
        $client = new ClientObject(
            "Client Name",
            new Pod([
                ...
                ModelB::class => function (int $arg1) {
                    return new PartB($arg1);
                }
            ])
        );

        /// See ClientObject which implements it as: $this->openDependency->new(ModelA::class, 1, "dous");
        $test = new TestIdentity;
        $this->assertTrue($client->productA()->identification()['number'] == $test->id());
        $this->assertTrue($client->partB()->number() == $test->id());
    }
    ...
    /** @test */
    public function it_does_not_use_open_creation_just_a_dictionary_of_closures()
    {
        /// This is a raw implementation. You can use this in place of OpenCreation.
        $client = new ClientObjectArray(
            "Client Name",
            [
                ModelA::class => function (int $arg1, string $arg2) {
                    return new ProductA($arg1, $arg2);
                },
                ModelB::class => function ($arg1) {
                    return new PartB($arg1);
                }
            ]
        );

        /// See ClientObjectArray which implements it as: $this->openDependency[ModelA::class](1, "dous");
        $test = new TestIdentity;
        $this->assertTrue($client->productA()->identification()['number'] == 1);
        $this->assertTrue($client->partB()->number() == 1);
    }
...
class ClientObject implements ClientModel
{
    ...
    public function partB() : ModelB
    {
        return $this->openDependency->new(ModelB::class, $this->test->id());
    }
}

class ClientObjectArray
{
    /// For testing a solution with the plain language.
    public function __construct(string $name, array $openDependency)
    {
        $this->name = $name;
    ...
    public function productA() : ModelA
    {
        return $this->openDependency[ModelA::class]($this->test->id(), $this->test->name());
    }

    public function partB() : ModelB
    {
        return $this->openDependency{ModelB::class}($this->test->id());
    }
}
